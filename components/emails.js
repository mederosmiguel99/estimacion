...
export default function emails({ email, onEdit, onDelete }) {
    return (
        <>
            <div key={email._id} className={`subcategory-card bg-white border border-gray-300 rounded-lg p-4 mb-3 dark:hover:border-fuchsia-300 transition`}>
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center space-x-2">
                        <span className={`badge ${email.isReceived ? "bg-green-300" : "bg-fuchsia-300"} text-primary-800 dark:text-primary-300`}>
                            {email.isReceived ? 'Received' : 'unReceived'}
                        </span>
